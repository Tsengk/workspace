...
roslaunch perception_calibration perception_cali_pt.launch
2. UI 交互说明
...
程序自动修改 params/camera_extrinsics.yaml，永久保存当前外参。

📂 关键文件说明
perception_cali_pt.launch: 启动配置与 Topic 重映射（订阅 /driver/lidar/rs_top_points 和 /gmsl_image0）。

camera_intrinsics.yaml: 存放相机内参矩阵、畸变系数 (plumb_bob 模型) 等。

camera_extrinsics.yaml: 存放 LiDAR 到相机的外参 (6 DoF 与 Quaternion)。本工具将自动修改此文件。
